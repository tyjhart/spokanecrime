...
```{r plot.offenses_over_time_violence, fig.cap = "Offenses Over Time, September 12, 2017 Onward"}
...
```{r plot.offenses_over_time_violence_wrap, fig.cap = "Offenses Over Time, September 12, 2017 Onward"}
daily_offenses_violence %>%
  ggplot(aes(date, offenses)) + 
  geom_point(size = 3, alpha = point_alpha) +
...
```{r plot.violent_offenses_over_time, fig.cap = "Offenses Over Time, September 12, 2017 Onward"}
daily_offenses_violence %>%
...
```{r plot.non_violent_offenses_over_time, fig.cap = "Offenses Over Time, September 12, 2017 Onward"}
daily_offenses_violence %>%
...
```{r plot.avg_daily_offenses_by_month, fig.cap = "Average Daily Offenses by Month"}
summarized_crimes %>%
...
```{r plot.2019_offenses_by_weekday, fig.cap = "Offenses by Day, 2019"}
df.crimes %>%
  filter(year == 2019) %>%
  count(dow) %>%
  ggplot(aes(fct_relevel(dow,"Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"), n, label = n)) +
...
```{r plot.2018_offenses_by_weekday, fig.cap = "Offenses by Day, 2018"}
df.crimes %>%
  filter(year == 2018) %>%
  count(dow) %>%
  ggplot(aes(fct_relevel(dow,"Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"), n, label = n)) +
